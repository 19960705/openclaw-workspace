...
## 📋 待办事项跟踪 (Todo Tracking)

### 核心流程
1. **实时捕获**: 每次对话中提到的待办事项，立即写入 `memory/todos.md`
2. **状态管理**: 
   - `待办 (Pending)` - 新增待办
   - `进行中 (In Progress)` - 开始执行时移动
   - `已完成 (Done)` - 完成后移动并添加日期
3. **每日提醒**: 每天 09:00 通过 cron 自动触发待办事项总结和汇报

### 使用说明
- 待办文件位置: `memory/todos.md`
- 自动提醒: 已设置每日 cron 任务 (0 9 * * * Asia/Shanghai)
- 每次会话开始前检查: 读取 `memory/todos.md` 确认当前待办
